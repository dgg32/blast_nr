...
STR=$(find $1 -maxdepth 1 -regex '\S+faa' | head -n $SLURM_ARRAY_TASK_ID | tail -n 1)
...
######
blastp -max_target_seqs 20 -db $2 -query $STR -num_threads 28 -task 'blastp' -outfmt '6 qaccver saccver pident qcovs length mismatch gapopen qstart qend sstart send evalue bitscore' -out $outfile
